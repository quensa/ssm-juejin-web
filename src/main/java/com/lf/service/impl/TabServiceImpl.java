...
import com.lf.service.TabService;
...
public class TabServiceImpl implements TabService {
    ...
    public String modifyTab(Tab tab) {
        ...
        if (tmpTab != null) {
            ...
            if (tmpForum != null) {
                ...
                resultStr = new String("修改失败：版块不存在！");
            }
        } else {
            resultStr = new String("修改失败：分类不存在！");
        }
        return resultStr;
    }
    ...
    /**
     * 逻辑删除分类* @param tab_id 分类id
     *
     * @return 0成功，-1分类不存在，-2删除失败，-3删除关联的文章失败
     */
    @Override
     ...
     *
     * @param tab_id 分类id
     * @return 0成功，-1分类不存在，-2取消删除失败，-3取消删除关联的文章失败
     */
    @Override
    ...
    private int deleteAllTipByTabId(int tabId) {
        Logger logger = Logger.getLogger(TabServiceImpl.class);
        // 查询关联的文章id
        logger.info("查询分类关联的文章，分类id：" + tabId);
        List<Integer> tipIdList = postMapper.selAllpostIdByTabId(tabId);
        int tipCount = tipIdList.size();
        // 删除关联的文章
        int result = postMapper.updAllpostIsDeletedByTabId(tabId);

        if (result == tipCount) {
            return 0; // 成功删除所有关联的文章
        } else if (result < tipCount && result > 0) {
            return -1; // 有部分文章未能成功删除
        } else if (result <= 0) {
            return -2; // 删除失败
        } else {
            return -99; // 删除失败，未知原因
    ...
    private int disDeleteAllTipByTabId(int tabId) {
        ...
        int tipCount = tipIdList.size();
        // 取消删除关联的文章
        int result = postMapper.updAllpostIsNotDeletedByTabId(tabId);

        if (result == tipCount) {
            return 0; // 成功取消删除所有关联的文章
        } else if (result < tipCount && result > 0) {
            return -1; // 有部分文章未能取消删除
        } else if (result <= 0) {
            return -2; // 取消删除失败
        } else {
